...
| `recall_related` | Walk the graph from a seed key with `step`, `k`, a `direction` (`out` default — forward BFS over out-edges; `in` — the seed's direct predecessors, so seeding a pure sink is no longer empty; `both` — union), and the orthogonal axes (`algorithm` ∈ `bfs` / `ppr` / `community`, `reduction` ∈ `none` / `mst` / `spt`, `objective` ∈ `min` / `max`, `weighting` ∈ `raw` / `tfidf` / `bm25`; see #410, #800, #801, #845, #961). `algorithm` selects the traversal **family** (`bfs` default; `ppr` runs Personalized PageRank — random-walk-with-restart proximity to the seed; `community` extracts the seed's local cluster) and the orthogonal `reduction` axis hands the `bfs`/`community` subgraph back as a tree rooted at the seed (e.g. `algorithm=community reduction=mst` returns the community as a minimum-spanning-tree backbone; `objective` sets the reduction direction). `reduction` is ignored for `ppr`. `restart_prob` (α) and `epsilon` (ε) tune the `ppr`/`community` random walk. Returns related facts with cumulative weights. Reverse predecessors carry `key` + `weight` only (no payload — call `recall_fact`); the reverse scan is bounded and sets `truncated` when it stops early. Read-only by default; pass `reinforce=true` to add a decaying weight pulse to the edges it traverses (opt-in — see below). |
