...
| `ScanVertices` | Enumerate vertices by key prefix, page-by-page | Opaque cursor; server enforces a default and hard cap on `limit` (see `LANTERN_SCAN_*`); ascending by default or descending via `order` (#898), with an **order-bound** cursor (replaying an ascending cursor under a descending scan, or vice versa, is rejected with `InvalidArgument`); cross-feeding a cursor from a different `Scan*` RPC is also rejected with `InvalidArgument`; SDK helper `ScanVerticesAll` returns an `iter.Seq2` that auto-paginates |
| `ScanVertexKeys` | Enumerate vertex **keys** (no values) by prefix, page-by-page | Keys-only, wire-efficient backing for the `keys` CLI verb; a non-empty `prefix` is REQUIRED (empty → `InvalidArgument`); ascending or descending via `order` (#898) with the same order-bound cursor rule as `ScanVertices`; its own opaque cursor kind (not interchangeable with other `Scan*`); reuses the `LANTERN_SCAN_*` clamps; SDK helper `ScanVertexKeysAll` auto-paginates |
...
| `TopVerticesByDegree` | Rank the most-connected live vertices under a key prefix | Read-only aggregate (#900); a non-empty `prefix` is REQUIRED (empty → `InvalidArgument`, so there is no whole-graph ranking); `direction` selects out / in / both degree (`DIRECTION_UNSPECIFIED` → out); `weighted` ranks by summed live edge weight instead of raw live edge count (both are returned per entry); honours live-visibility (#750) — expired vertices and decayed edges don't count; `k` is clamped by the shared `LANTERN_SCAN_*` limits; results are descending and point-in-time best-effort (like `GetServerStatus` counts); server-side only — no SDK wrapper |
