...
func TestReceiptBaselineSidecarPersistsVerifiesAndCleansOrphans(t *testing.T) {
	...
	if digest != sha256.Sum256(raw) ||
		path != receiptBaselineSidecarPath(walPath, ReceiptBaselineFormatCombinedV2, digest) {
		t.Fatalf("persisted sidecar = %x %q", digest, path)
	}
	got, err := store.load(ReceiptBaselineFormatCombinedV2, digest, uint64(len(raw)))
	if err != nil || !bytes.Equal(got, raw) {
		t.Fatalf("loaded sidecar = %q, %v", got, err)
	}
	if secondDigest, secondPath, err := store.persist(ReceiptBaselineFormatCombinedV2, raw); err != nil ||
		secondDigest != digest || secondPath != path {
		t.Fatalf("idempotent persist = %x %q, %v", secondDigest, secondPath, err)
	}

	orphanRaw := []byte("orphan")
	orphanDigest, orphanPath, err := store.persist(ReceiptBaselineFormatCombinedV2, orphanRaw)
	if err != nil {
		t.Fatal(err)
	}
	temp := receiptBaselineTempPrefix(walPath, ReceiptBaselineFormatCombinedV2, orphanDigest) + "stale"
	...
	zeroDigestPath := receiptBaselineSidecarPath(
		walPath,
		ReceiptBaselineFormatCombinedV2,
		[sha256.Size]byte{},
	)
...
func TestReceiptBaselineSidecarFailsClosedOnDamageAndFaults(t *testing.T) {
	t.Run("before rename", func(t *testing.T) {
		...
		if _, _, err := store.persist(ReceiptBaselineFormatCombinedV2, raw); err == nil {
	...
	t.Run("after rename", func(t *testing.T) {
		...
		if gotDigest, gotPath, err := retry.persist(ReceiptBaselineFormatCombinedV2, raw); err != nil ||
			gotDigest != digest ||
			gotPath != receiptBaselineSidecarPath(walPath, ReceiptBaselineFormatCombinedV2, digest) {
	...
	t.Run("corrupt truncate oversized and symlink", func(t *testing.T) {
		...
		if _, err := store.load(ReceiptBaselineFormatCombinedV2, digest, uint64(len(raw))); !errors.Is(err, errReceiptBaselineSidecar) {
			t.Fatalf("truncated sidecar = %v", err)
		}
		if _, err := store.load(
			ReceiptBaselineFormatCombinedV2,
			digest,
			maxReceiptBaselineV2Bytes+1,
