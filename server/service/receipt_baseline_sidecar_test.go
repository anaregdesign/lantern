...
func TestReceiptBaselineSidecarPersistsVerifiesAndCleansOrphans(t *testing.T) {
	...
	digest, path, err := store.persist(raw)
	if err != nil {
		t.Fatal(err)
	}
	if digest != sha256.Sum256(raw) || path != receiptBaselineSidecarPath(walPath, digest) {
		t.Fatalf("persisted sidecar = %x %q", digest, path)
	}
	got, err := store.load(digest, uint64(len(raw)))
	if err != nil || !bytes.Equal(got, raw) {
		t.Fatalf("loaded sidecar = %q, %v", got, err)
	}
	if secondDigest, secondPath, err := store.persist(raw); err != nil ||
		secondDigest != digest || secondPath != path {
		t.Fatalf("idempotent persist = %x %q, %v", secondDigest, secondPath, err)
	}

	orphanRaw := []byte("orphan")
	orphanDigest, orphanPath, err := store.persist(orphanRaw)
	if err != nil {
		t.Fatal(err)
	}
	temp := receiptBaselineTempPrefix(walPath, orphanDigest) + "stale"
	...
	if err := store.cleanup(digest); err != nil {
		t.Fatal(err)
	}
	...
	zeroDigestPath := receiptBaselineSidecarPath(walPath, [sha256.Size]byte{})
	if err := os.WriteFile(zeroDigestPath, []byte("zero orphan"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := store.cleanup([sha256.Size]byte{}); err != nil {
		t.Fatal(err)
	}
...
func TestReceiptBaselineSidecarFailsClosedOnDamageAndFaults(t *testing.T) {
	t.Run("before rename", func(t *testing.T) {
		...
		if _, _, err := store.persist(raw); err == nil {
			t.Fatal("before-rename fault succeeded")
		}
		if _, err := os.Stat(receiptBaselineSidecarPath(walPath, digest)); !errors.Is(err, os.ErrNotExist) {
	...
	t.Run("after rename", func(t *testing.T) {
		...
		if got, err := (receiptBaselineSidecarStore{walPath: walPath}).load(digest, uint64(len(raw))); err != nil ||
			!bytes.Equal(got, raw) {
			t.Fatalf("renamed orphan is not valid: %q, %v", got, err)
		}
		retry := receiptBaselineSidecarStore{walPath: walPath}
		if gotDigest, gotPath, err := retry.persist(raw); err != nil ||
			gotDigest != digest || gotPath != receiptBaselineSidecarPath(walPath, digest) {
	...
	t.Run("corrupt truncate oversized and symlink", func(t *testing.T) {
		...
		if _, err := store.load(digest, uint64(len(raw))); !errors.Is(err, errReceiptBaselineSidecar) {
			t.Fatalf("truncated sidecar = %v", err)
		}
		if _, err := store.load(digest, maxReceiptBaselineBytes+1); !errors.Is(err, errReceiptBaselineSidecar) {
