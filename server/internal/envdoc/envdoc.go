...
var descriptions = map[string]string{
	...
	"LANTERN_MAX_VERTICES":                "Soft local-admission cap on the conservative vertex footprint: live vertices plus retained Put barriers (0 = unlimited). Charging born-expired Put barriers prevents a cap bypass. This intentionally overlaps the separate causal-identity budget; replication apply and backup restore bypass it, and edge admission conservatively assumes both endpoints may be new.",
	"LANTERN_MAX_EDGES":                   "Soft local-admission cap on the conservative edge footprint: live edges plus retained Put barriers (0 = unlimited). A live additive edge coexisting with a barrier counts twice, and charging born-expired Put barriers prevents a cap bypass. This intentionally overlaps the separate causal-identity budget; replication apply and backup restore bypass it.",
	"LANTERN_MAX_VERTEX_CAUSAL_ENTRIES":   "Hard atomic local-origin admission budget over the exact retained vertex causal-identity union (live HLC floor, Put barrier, or Delete tombstone); 0 = unlimited. Replication apply bypasses the limit for convergence and can report over-limit.",
	"LANTERN_MAX_EDGE_CAUSAL_ENTRIES":     "Hard atomic local-origin admission budget over the exact retained edge causal-identity union (live HLC floor, Put barrier, or Delete tombstone); 0 = unlimited. Replication apply bypasses the limit for convergence and can report over-limit.",
	...
	"LANTERN_SEARCH_POSITIONS":              "Record positional postings so phrase queries verify adjacency and the proximity boost ranks tight matches higher. Off drops the position store: phrase=true fails closed with FAILED_PRECONDITION / SEARCH_POSITIONS_DISABLED instead of returning unordered matches.",
	...
	"LANTERN_SEARCH_TIMEOUT_MS":             "Server-side deadline applied to every SearchVertices attempt.",
	"LANTERN_SEARCH_MAX_QUERY_BYTES":        "Maximum UTF-8 bytes accepted in one search query.",
	"LANTERN_SEARCH_MAX_QUERY_TERMS":        "Maximum distinct analyzed terms in one search query.",
	...
	"LANTERN_SEARCH_CURSOR_TTL_SECONDS":     "Lifetime of an endpoint-sticky bounded search session and its signed cursors.",
	"LANTERN_SEARCH_MAX_SESSIONS":           "Maximum live search pagination sessions retained by one endpoint; least-recently-used sessions are evicted.",
	"LANTERN_SEARCH_MAX_SESSION_HITS":       "Maximum ranked hits retained by one search session; must exceed LANTERN_SEARCH_MAX_LIMIT.",
	"LANTERN_SEARCH_MAX_SESSION_BYTES":      "Aggregate byte budget for retained search-session hits on one endpoint.",
	...
	"LANTERN_SEARCH_MAX_DOCUMENT_TERMS":     "Maximum distinct analyzed terms in one search document.",
	"LANTERN_SEARCH_MAX_LIVE_TERMS":         "Maximum distinct terms retained across the live search index.",
	"LANTERN_SEARCH_MAX_LIVE_POSTINGS":      "Maximum live term-document posting entries.",
	"LANTERN_SEARCH_MAX_POSITION_ENTRIES":   "Maximum live positional entries when search positions are enabled.",
	"LANTERN_SEARCH_COMPACTION_RATIO":       "Retained-to-live high-water ratio that triggers index compaction.",
	"LANTERN_SEARCH_COMPACTION_MIN_RETIRED": "Minimum retired term/ordinal slots before ratio-triggered compaction.",
...
func Render(specs []envconfig.Spec) (string, error) {
	...
	b.WriteString("local or relayed mutation is published, Lantern verifies that its exact binary\n")
	b.WriteString("full-mutation `SubscribeResponse` and the largest receiver-local relay of any\n")
	b.WriteString("receipt envelope fit `LANTERN_MAX_SEND_MSG_BYTES`; an oversize frame is rejected\n")
	b.WriteString("with `ResourceExhausted` before graph, Store, WAL, or origin publication.\n")
	b.WriteString("Startup also fails if a retained frame or its maximal relay exceeds a newly\n")
	b.WriteString("lowered send cap. `LANTERN_MAX_RECV_MSG_BYTES` remains an independent\n")
	b.WriteString("request cap, so a request that fits it may still be rejected after response\n")
	b.WriteString("projection. Receipt-WAL and receipt-wire 8 MiB bounds do not raise the send cap;\n")
	b.WriteString("intrinsic wire limits apply even when the configured cap is unlimited. Every\n")
	b.WriteString("full-mutation Subscribe consumer must accept its sender's admitted maximum;\n")
