...
import { ReplicationPeer_State } from "lantern-sdk/web";

/**
 * ReplicationPeerState is the JSON-friendly state label the Ops view
 * displays as a pill colour. Mirrors graph.v1.ReplicationPeer.State
 * but collapses the proto enum to a lowercase string so React state
 * stays primitive.
 */
...
/**
 * Calls LanternService.GetReplicationStatus via `lantern-sdk/web`
 * and normalises the response into the flat ReplicationStatus shape
 * used by the Ops view (#409).
 */
export async function getReplicationStatus(
  ...
  try {
    const resp = await client.getReplicationStatus(init?.signal);
