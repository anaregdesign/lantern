...
| `sdks/dart/offline/` | `lantern_client_offline` (experimental Dart package) | Opt-in storage-neutral confirmed cache, durable Put/Add outbox, pending overlays, and foreground replay. Depends on `lantern_client` by path while unpublished; excluded from the parent publish archive. |
...
- **Dependency boundaries (enforce as invariants)**:
  ...
  - `sdks/dart/offline/` is a separate experimental pure-Dart package. The online parent never imports it. It injects `OfflineStore`, keeps credentials/encryption/OS scheduling application-owned, admits only replay-safe Put and stable-ID Add mutations, and has no production database adapter. Its `InMemoryOfflineStore` and canonical snapshots are conformance/test infrastructure only.
