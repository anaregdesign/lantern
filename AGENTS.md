...
- **Dart release gate**: `.github/workflows/dart-sdk.yml` is path-filtered on `sdks/dart/**`, `proto/**`, Buf pins, and itself. It tests the documented Dart floor plus the Flutter version pinned by `docs/decisions/0001-dart-mobile-transport.md`, checks actual `lib/src/gen` drift, real wire, warning-free dartdoc, example Android/iOS conformance, publish dry-run, and uploads a pinned `pana` report. Dedicated offline jobs run its minimum/current Dart gates, fresh-process snapshot verification, real-server committed-response-loss replay, and parent archive isolation; they never publish the experimental child. `sdks/dart/vX.Y.Z` publishes `lantern_client` with pub.dev OIDC only after every job passes, then creates an exact-title GitHub Release. There is no long-lived pub token.
