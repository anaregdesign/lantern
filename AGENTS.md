...
| `sdks/dart/offline/` | `lantern_client_offline` (experimental Dart package) | Opt-in storage-neutral confirmed cache, durable Put-only outbox, pending overlays, legacy-Add quarantine, and foreground replay. Uses the hosted `lantern_client` package and has an independent release tag; excluded from the parent publish archive. |
...
| `go.work` | — | Pins all 6 Go modules for local dev. Dart and TypeScript packages live outside `go.work`. |
...
- **Providers**: [server/provider/provider.go](server/provider/provider.go) assembles the aggregate `Config` plus a set of focused sub-configs — `NetConfig`, `TLSConfig`, `RateLimitConfig`, `ObservabilityConfig`, `CacheConfig` (incl. the #848 capacity caps), `ShutdownConfig`, `TraversalConfig`, `AuthConfig` (#850), `ScanConfig`, `SearchConfig`, `ReceiptWALConfig`, `LLMConfig` (#828), … — read from env vars (`LANTERN_PORT`, `LANTERN_DEFAULT_TTL_SECONDS`, `LANTERN_METRICS_ADDR`, TLS material, rate-limit knobs, etc.). `NewServingRuntime` is the sole graph-only versus private durable receipt-WAL state boundary; it must certify and install one owned graph/Store/origin/Log/HLC bundle before listeners and replication workers are constructed. Each provider takes the **slice it actually needs** (for example, `NewListener` takes `NetConfig` plus the private runtime-certification marker), not `*Config` — keep that SRP invariant when adding providers. The full env-var contract lives in [server/internal/envconfig](server/internal/envconfig). `App` / `main` may still hold `*Config` because they observe multiple slices.
...
- **Dependency boundaries (enforce as invariants)**:
  ...
  - `sdks/dart/offline/` is a separate experimental pure-Dart package. The online parent never imports it. It injects `OfflineStore`, keeps credentials/encryption/OS scheduling application-owned, and admits only replay-safe Put mutations. `OfflineStoreTransaction` operations return `FutureOr<T>` and callers must await them. Legacy Add records remain decodable only for fail-closed `unsupported_add` terminal migration and authorized inspection; durable Add stays disabled until #1115 supplies server-authoritative operation receipts and matching evidence. Its `InMemoryOfflineStore` and canonical snapshots are conformance/test infrastructure only.
