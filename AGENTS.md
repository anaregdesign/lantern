...
- **Receipt Add result boundary (#1115/#1398/#1453)**: public Put/Add edge source weights must be finite at server ingress; receipt-aware SDK Add paths also prevalidate outbound sources, but Go receipt-less Add forwards its `float32` weight for server validation. Replay/status must preserve the authoritative original effective `float32` result bytes even when finite sources sum to `+Infinity`/`-Infinity` or reflect pre-existing accepted `NaN`; SDKs must not turn a `CONFIRMED` `NaN` into unknown or zero.
