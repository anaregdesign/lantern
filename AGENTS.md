...
This project used to be split across four separate repositories (`lantern` / `lantern-proto` / `lantern-cli` / `papaya`), but **everything is now consolidated into this repo**.
...
| `pkg/papaya/` | former [`papaya`](https://github.com/anaregdesign/papaya) | Core graph / cache / NLP algorithms |
...
- **Providers**: [server/provider/provider.go](server/provider/provider.go) assembles `Config` (env vars `LANTERN_PORT`, `LANTERN_DEFAULT_TTL_SECONDS`), `net.Listener`, `grpc.Server`, and `papaya/cache/graph.GraphCache`. `NewListener` now receives the wire-injected `*Config`.
